...
if __name__ == '__main__':
    ...
    for similarity in [100,95,90,70,50]:
        dataset_name = f'similarity_{similarity}'

        ################################################################
        # set up experiemnt
        ################################################################

        dataset = ContactCorrPers8A(set_name=dataset_name,
                                    go_thres=25,
                                    entry_type='chain')
        ...
        exp = ProDAR_Experiment(
            model,
            name_suffix=f'{dataset.set_name}-GOthres{dataset.go_thres}-amd',
            rand_seed=69
        )
        ...
        exp.train_split(n_epochs=600, train_valid_ratio=0.9, batch_size=256)
